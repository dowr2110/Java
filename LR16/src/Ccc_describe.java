...
public class Ccc_describe extends HttpServlet implements Servlet {

    CBean Cb;
    public void init() throws ServletException {
        //  Сервлет Ссс  создает объект  класса CBean  при инициализации,
        //  а ссылку на объект записывает в атрибут контекста с именем  atrABean
        super.init();
    ...
    protected void service(HttpServletRequest rq, HttpServletResponse rs)
        ...
        if(rq.getMethod().equals("GET") || rq.getMethod().equals("POST"))//определяем get или post метод(ТУТ NEW)
        {
            if(rq.getParameter("CBean").equals("new")) {//определяем параметр запроса: содержит ли old или new
                //созваем новый обьект класса CBean
